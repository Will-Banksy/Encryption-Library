...
#define NUM_THREADS std::thread::hardware_concurrency() == 0 ? 8 : std::thread::hardware_concurrency()
		...
		std::vector<uint128_t> counterBlocks;
		counterBlocks.resize(blocks.size());
		counterBlocks[0] = counter;
		for(size_t i = 1; i < counterBlocks.size(); i++) {
			counterBlocks[i] = counterBlocks[i - 1] + 1;
		}
...
std::vector<RoundKey> AESEncryption::GenRoundKeys128(const std::vector<uint8_t>& key, Rand* rand) { // TODO Should probably remove rand
	...
	for(uint8_t i = 1; i < roundKeys.size(); i++) {
		roundKeys.at(i).w0 = roundKeys.at(i - 1).w0 ^ std::rotl(roundKeys.at(i - 1).w3, 8) ^ c_Rcon128[i];
...
void AESEncryption::EncryptBlock128(Block<128>& block, const std::vector<RoundKey>& roundKeys, bool doAesni) {
...
void AESEncryption::ShiftRows128(Block<128>& data, const RoundKey& roundKey) {
	for(uint8_t y = 0; y < data.c_BlockDims; y++) {
		uint8_t shift = y;
		if(shift > 0) {
			std::cout << "\nShiftLeft by " << (uint16_t)shift << std::endl;
			std::cout << (uint16_t)data(0, y) << ", " << (uint16_t)data(1, y) << ", " << (uint16_t)data(2, y) << ", " << (uint16_t)data(3, y) << std::endl;
